...
interface CombinedWheelVisualizerProps {
...
const CombinedWheelVisualizer: React.FC<CombinedWheelVisualizerProps> = ({
  movies,
  hoveredMovieId,
  size = 260,
}) => {
  ...
  const draw = () => {
    ...
    const nMovies  = movieList.length;
    ...
    // Clip path = union of all petals — keeps everything inside wheel
    const clipId = "wheel-clip";
    ...
    // ── LAYER 2: One spider per movie ─────────────────────────────────────────
    const focusId  = hoveredMovieId ?? null;
    const anyFocus = focusId !== null;

    // Draw non-hovered first, hovered last so it renders on top
    const sorted = [...movieList].sort((a, b) => {
      if (!anyFocus) return 0;
      return a.id === focusId ? 1 : b.id === focusId ? -1 : 0;
    });

    sorted.forEach((movie, mi) => {
      const origIdx = movieList.findIndex(m => m.id === movie.id);
      const isFocused = !anyFocus || movie.id === focusId;

      // Depth: back spiders bigger+faded, front spiders smaller+bright
      const depthScale = 1 + (nMovies - 1 - origIdx) * 0.10;
      const fillOp  = !anyFocus ? (0.06 + origIdx * (0.20 / nMovies)) : isFocused ? 0.35 : 0.12;
      const alpha   = !anyFocus ? (0.30 + origIdx * (0.65 / nMovies)) : isFocused ? 1.00 : 0.45;
      ...
      // Tiny angular offset so spiders fan out and are distinguishable
      ...
      // Spider group with drop shadow
      const spG = g.append("g")
        .attr("filter", !anyFocus || isFocused ? "url(#sp-shadow)" : "none");

      // 1. Colored sector fills — clipped inside wheel boundary
      const fillG = spG.append("g").attr("clip-path", `url(#${clipId})`);
      pts.forEach((a, i) => {
        const sa   = i * sliceAng - Math.PI / 2 + angleOff;
        const ea   = sa + sliceAng;
        ...
        fillG.append("polygon")
          ...
          .attr("fill",  isGray ? "#aaaaaa" : a.em.color)
      ...
      pts.forEach((a, i) => {
        ...
        outlineG.append("path")
          ...
          .attr("fill", "none")
          .attr("stroke", isGray ? "#cccccc" : a.em.color)
      ...
      pts.forEach((p) => {
        spG.append("circle")
          ...
          .attr("r", dotR)
          .attr("fill", isGray ? "#aaaaaa" : p.em.color)
          .attr("stroke", "#fff")
          .attr("stroke-width", 1)
          .attr("opacity", alpha);
      });
